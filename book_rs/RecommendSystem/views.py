...
def search_authors(request, page_no: int):
    ...
    text = request.GET.get("text")
    authors = list(
        Author.objects.filter(firstname__iregex=text)
        | Author.objects.filter(lastname__iregex=text)
        | Author.objects.filter(patronymic__iregex=text)
    )
...
def add_favorite(request, book_id: int):
    ...
    if user.is_authenticated and book:
        ...
        if book.in_black_list(user.id):
            HttpResponsePermanentRedirect(
                reverse("RS:delete_black_list", kwargs={"book_id": book.id})
            )
        return HttpResponsePermanentRedirect(
            reverse("RS:favorite", kwargs={"page_no": 1})
        )
    else:
        return render(
            request,
...
def add_black_list(request, book_id: int):
    ...
    if user.is_authenticated and book:
        ...
        if book.in_favorite(user.id):
            HttpResponsePermanentRedirect(
                reverse("RS:delete_favorite", kwargs={"book_id": book.id})
            )
        return HttpResponsePermanentRedirect(
            reverse("RS:black_list", kwargs={"page_no": 1})
        )
    else:
        return render(
            request,
...
def add_comment(request, book_id: int):
    if request.method == "POST":
        ...
        if user.is_authenticated and get_object_or_404(Book, pk=book_id):
            ...
            comment.save()
            return HttpResponsePermanentRedirect(
                reverse("RS:show_one_book", kwargs={"book_id": book_id})
            )
        else:
            return render(
                request,
